rem
rem This Batch script will build a production environment for the JQuizzer application
rem This script only depends on the rest of JQuizzer, and being in its root directory
rem This script will create a prod directory, bundle dependencies, and create run scripts to run JQuizzer
rem
...
rem %~dp0 gets the current directory because of batch weirdness. This script expects to run within the root JQuizzer dir
set workdir=%~dp0
cd %workdir%
...
echo Win JDK has been copied over
...
rem %% is needed, as % has to be escaped in a special way. Normal escapes use ^ but not %
